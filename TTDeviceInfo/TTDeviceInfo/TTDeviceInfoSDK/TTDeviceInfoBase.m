...
@implementation TTDeviceInfoBase

#define TTService [[NSBundle mainBundle] bundleIdentifier] ?:@"TTDeviceInfo"

+ (NSString *)uuid {
    NSString * uuid = NULL;
    uuid = [KeychainWrapper searchDateWithService:TTService];
    if (uuid) {
        return uuid;
    }else{
        uuid = [[NSUUID UUID] UUIDString];
        if([KeychainWrapper saveDate:uuid withService:TTService]){
            return uuid;
        }else{
            return NULL;
        }
...
+ (NSString *)carrierCode {
    ...
    if(!carrier.isoCountryCode){
        ...
        mobile = [carrier mobileNetworkCode];
    }
    return mobile;
}
